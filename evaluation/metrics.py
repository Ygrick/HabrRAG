...
def compute_hit_at_k(items: Sequence[EvaluatedItem], ks: Sequence[int]) -> Dict[str, float]:
    ...
    return {f"hit@{k}": round(hit_counts[k] / total, 6) for k in ks}
...
def compute_recall_at_k(items: Sequence[EvaluatedItem], ks: Sequence[int]) -> Dict[str, float]:
    ...
    for example in items:
        ...
        for k in ks:
            ...
            recall_counts[k] += 1
    return {
        f"recall@{k}": round(recall_totals[k] / (recall_counts[k] or 1), 6)
        for k in ks
    }
...
async def _average_metric_score(
...
async def compute_ragas_retriever_metrics(samples: Optional[SingleTurnSample]) -> Dict[str, float]:
    if not samples:
        return {
    ...
    results["id_based_context_recall"] = await _average_metric_score(IDBasedContextRecall(), samples)
    results["id_based_context_precision"] = await _average_metric_score(IDBasedContextPrecision(), samples)

    return results


async def compute_ragas_generator_metrics(
    ...
    results["bleu_score"] = await _average_metric_score(BleuScore(), samples)
    results["rouge_score_1_fmeasure"] = await _average_metric_score(RougeScore(rouge_type="rouge1", mode="fmeasure"), samples)
    results["rouge_score_l_fmeasure"] = await _average_metric_score(RougeScore(rouge_type="rougeL", mode="fmeasure"), samples)
    results["similarity_scores"] = await _average_metric_score(
        AnswerSimilarity(embeddings=HuggingFaceEmbeddings(model=hf_model)), samples)

    results["chrf_score"] = await _average_metric_score(ChrfScore(), samples)
    results["non_llm_string_similarity"] = await _average_metric_score(NonLLMStringSimilarity(), samples)

    if llm:
        results["factual_correctness"] = await _average_metric_score(FactualCorrectness(llm=llm), samples)
        results["faithfulness"] = await _average_metric_score(Faithfulness(llm=llm), samples)
